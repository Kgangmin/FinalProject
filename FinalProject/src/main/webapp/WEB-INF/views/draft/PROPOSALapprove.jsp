	        ...
	        <div class="ef-approvals">
	          <c:forEach var="line" items="${approvalLine}" varStatus="st">
	            <div class="ef-approval-item">
	              ...
	              <c:choose>
	                <c:when test="${isMyLine and (line.approval_status eq '대기' or empty line.approval_status)}">
	                  <input type="hidden" name="approval_line_no" value="${line.approval_line_no}" />
	                  <div class="ef-field ef-colspan-2">
	                    <span class="ef-label">결재 의견</span>
	                    <textarea class="ef-approval-comment-textarea" name="approval_comment"
	                              placeholder="결재 의견을 입력하세요.">${line.approval_comment}</textarea>
	                  </div>
	                </c:when>
	                <c:otherwise>
