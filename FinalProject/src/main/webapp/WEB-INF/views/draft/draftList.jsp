...
<div class="container-fluid" >
  <div class="row" >
    ...
    <!-- 좌측 2차 사이드바 (다우오피스 느낌: 어두운 배경, 섹션 구분) -->
    <aside class="col-auto bg-dark text-white border-end min-vh-100" style="width:220px;" >
      <div class="p-3 d-flex align-items-center border-bottom border-secondary" >
       
        
      </div>

      <div class="px-3 pt-3 pb-1 small text-white-50">신청 기능</div>
      <nav class="nav flex-column mb-2">
        <a class="nav-link text-white-50 py-2" href="${pageContext.request.contextPath}/draft/write">신청하기</a>
        <a class="nav-link text-white py-2 bg-dark" href="${pageContext.request.contextPath}/draft/list">나의 신청목록
          <c:if test="${counts.inProgress gt 0}">
            <span class="badge rounded-pill bg-danger ms-2">${counts.inProgress}</span>
          </c:if>
        </a>
      </nav>

      <div class="px-3 pt-3 pb-1 small text-white-50">승인 기능</div>
      <nav class="nav flex-column mb-2">
        <a class="nav-link text-white-50 py-2" href="${pageContext.request.contextPath}/draft/approve">승인하기</a>
        <a class="nav-link text-white-50 py-2" href="${pageContext.request.contextPath}/draft/shared">공유 · 열람가능</a>
      </nav>

      <div class="px-3 pt-3 pb-1 small text-white-50">관리자 기능</div>
      <nav class="nav flex-column mb-4">
        <a class="nav-link text-white-50 py-2" href="${pageContext.request.contextPath}/draft/admin/list">모든 신청목록</a>
        <a class="nav-link text-white-50 py-2" href="${pageContext.request.contextPath}/draft/admin/files">모든 파일목록</a>
        <a class="nav-link text-white-50 py-2" href="${pageContext.request.contextPath}/draft/admin/settings">기본정보 설정</a>
      </nav>
    </aside>

    <!-- 본문 -->
    <main class="col p-4">
      <!-- 가운데 고정폭(다우오피스 본문 폭 느낌) -->
      <div class="mx-auto" style="max-width: 1080px;">

        <!-- 상단: 제목 + 검색/다운로드 -->
        <div class="d-flex align-items-start justify-content-between mb-3">
          <div>
            <h4 class="fw-bold mb-1">나의 신청목록</h4>
            <div class="text-muted small">내가 진행한 신청의 목록입니다. 검색 화면에서 신청 검색이나 CSV 다운로드를 할 수 있습니다.</div>
          </div>
          <div class="d-flex gap-2" style="max-width: 460px;">
            <form class="input-group" action="${pageContext.request.contextPath}/draft/list" method="get">
              ...
              <div class="d-flex w-100 justify-content-between">
                <div class="pe-3">
                  <div class="fw-semibold">
                    <span class="text-body-secondary">[${doc.formName}]</span>
                    ${doc.title}
                  </div>
                ...
                <div class="text-nowrap d-flex align-items-center">
                  <span class="badge rounded-pill
                    ${doc.status=='진행중' ? 'text-bg-success' :
                      doc.status=='반려' ? 'text-bg-danger' :
                      doc.status=='임시보관' ? 'text-bg-secondary' :
                      doc.status=='완료' ? 'text-bg-primary' : 'text-bg-light'}">
                    ${doc.status}
                  </span>
                </div>
              </div>
